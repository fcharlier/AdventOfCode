...
def cell_name(cplx):
    # return "%d,%d" % (robot["pos"].real, robot["pos"].imag)
    return cplx


def paint(robot, white_cells, color):
    cell = cell_name(robot["pos"])
    white_cells[cell] = color
...
if __name__ == "__main__":
    ...
    x = [c.real for c in white_cells if white_cells[c] == 1]
    y = [c.imag for c in white_cells if white_cells[c] == 1]

    plt.figure(figsize=(4, 4))
    ax_scatter = plt.axes([.1, .1, .65, .65])
    ax_scatter.tick_params(direction='in', top=True, right=True)
    ax_scatter.scatter(x, y)
    ax_scatter.set_xlim((min(x) - 50, max(x) + 50))
    ax_scatter.set_ylim((min(y) - 50, max(y) + 50))
    plt.show()
