...
def max_rock_depth(lines):
    ...
    11
    """
    return max(point[1] for line in lines for point in line) + 2
...
def get_cave_map(lines):
    ...
    # Given than the maximum slope is 1:1,
    # extending the map by `depth` on each side is enough to simulate infinity
    left -= depth
    right += depth
